...
const updateBook = async function (req, res) {
    try {
        ...
        if (!bookId) return res.status(404).send({ status: false, msg: "No such Book Exits" });
        ...
        if (title) {
            ...
            if (titleCall) return res.status(400).send({ status: false, msg: `Book with name ${title}  is Already Present` });
        ...
        if (ISBN) {
            ...
            if (ISBNCall) return res.status(400).send({ status: false, msg: `Book with ISBN  ${ISBN}  is Already Present` })
